...
WandCastSession（纯 Java）
  ...
  TriggerPlan / PayloadPlan / ProjectilePlan（冻结 nodePath）
       ...
       v
CastTransaction（reserve 前生成 executionId，预检 root Trigger 容量）
  -> MinecraftEffectExecutor（服务器提交后只消费同一 identity）
...
每个接受的 cast 在 `CastTransaction` 的预算预留前得到服务器权威
...
与事件占用。现有 evaluator 的静态树默认仍以 32 个实体与 32 个 release event
作为保守 guard；`CastTransaction` 会再用中央每次施法的
`AUTHORITATIVE_ENTITIES` 与 `TRIGGER_RELEASES` ceiling 调用纯
`RootTriggerBudgetAllocator`。无法为 root 加上已冻结子树分配足够本地余额时，
它在 reservation 和 WandState commit 前以结构化预算拒绝退出。运行时的
`TriggerRuntimeBudget` 因而从同一个配置 ceiling 分配，最大硬上限为 128，
而不是在实体创建或存档重载后回落为 legacy 32/32。一次释放后，
...
这是树内、正常服务器生命周期的上限。G05 的 `SpellBudgetManager` 还同时
约束 owner、chunk、dimension、global、tick 和 window；根投射物使用已提交
local slice，延迟 Trigger 在实际位置重新申请中央额度。跨 chunk runtime
查询以一个原子请求向每个覆盖 chunk 保守预留完整结果上限，不能只把边界工作
归属 parent 所在 chunk。

## NBT 与兼容性

Schema v5 的冻结 payload 保存：

- 执行 UUID、实例 nodePath、catalog epoch/hash；
- 完整 `DamageProfile`；旧 `Damage` 标量只作为 `PROJECTILE` 兼容镜像；
...
与 runtime budget；v4 引入 `DamageProfile`，当前 v5 根节点要求所有后代继续使用 v5。
所有节点与根共享非零 execution UUID 和 catalog epoch/hash。实体 v3+ 还必须有 FrozenPayload 和 TriggerRuntimeState，
不得退回 legacy/fresh 状态。任一子节点的未来或降级 schema、非法枚举、重复路径、
不一致的身份、非法 item path 或损坏字段都会拒绝整棵树，实体安全移除而不释放。
冻结 NBT 可保存的爆炸半径上限是 32，但 G05 `WorldMutationPolicy` 的实际
Explosion 执行上限更保守地固定为 16；trail light 为 16、bounce 为 32。
v2 的 `DEATH` 显式迁移为 `EXPIRATION`；旧
`TriggerPayloadReleased=true` 保守迁移为 inert，优先避免重放，即使这会
停止旧 Piercing Trigger 的后续释放。

v5 实体只写 `FrozenPayload` 与 `TriggerRuntimeState` 这一份权威状态，
...
catalog 标识和唯一后代路径，随后才能按 v5 保存；v3+ 解码不接受全零 legacy UUID。
...
- `FrozenTriggerPayloadNbtTest`：最大深度/宽树往返、损坏子树、v3/v4/v5 缺字段、
  重复路径、混合 schema/identity、未来 schema、数值/路径安全、`DEATH`
  迁移、64 项 modifier 列表、legacy runtime 路径绑定和运行状态往返。
- Fabric GameTest：真实 Spark/Bomb 方块与实体 Hit（Bomb 实体 Hit 强制跨
  chunk seam）、连续 Piercing Hit、
